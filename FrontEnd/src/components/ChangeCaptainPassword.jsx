...
const ChangeCaptainPassword = ({ setChangeCaptainPasswordPanel, darkMode, showPopup}) => {
    const [oldPassword, setOldPassword] = useState("");
    const [newPassword, setNewPassword] = useState("");
  
    async function submitCaptainPasswordChange(e) {
    e.preventDefault();
    try {
      const response = await axiosCaptainInstance.post("/captains/change-password",
        {
          oldPassword,
          newPassword,
        },
    ...
    return (
      <div className={`max-w-md mx-auto px-6 py-7 ${darkMode ? "bg-[#1B1B1B] text-white" : "bg-gray-100"} min-h-screen`}>
        ...
        <form onSubmit={submitCaptainPasswordChange} className="space-y-6">
          <div>
